...
HEADER_PATTERNS = tuple((re.compile(p), h)
                        for p, h in CUMULUS.get("HEADERS", {}))
...
class SwiftclientStorage(Storage):
    ...
    def __init__(self, username=None, api_key=None, container=None,
        ...
        if CUMULUS["USE_PYRAX"]:
            if CUMULUS["PYRAX_IDENTITY_TYPE"]:
                pyrax.set_setting(
                    "identity_type", CUMULUS["PYRAX_IDENTITY_TYPE"])
            if CUMULUS["AUTH_URL"]:
            ...
            cls = pyrax.utils.import_class(
                'pyrax.identity.rax_identity.RaxIdentity')
            pyrax.identity = cls()
    ...
    def _get_container(self):
        ...
        if not hasattr(self, "_container"):
            if CUMULUS["USE_PYRAX"]:
                self._container = self.connection.create_container(
                    self.container_name)
            else:
