...
class Status(object):
  ...
  '''
    ...
    def AsDict(self):
        ...
        if self.urls:
            data['urls'] = dict([(url.url, url.expanded_url)
    ...
    def NewFromJsonDict(data):
        ...
        if 'entities' in data:
            if 'urls' in data['entities']:
                urls = [Url.NewFromJsonDict(u)
                        for u in data['entities']['urls']]
            if 'user_mentions' in data['entities']:
                user_mentions = [User.NewFromJsonDict(
                    u) for u in data['entities']['user_mentions']]
            if 'hashtags' in data['entities']:
                hashtags = [Hashtag.NewFromJsonDict(
                    h) for h in data['entities']['hashtags']]
        ...
        return Status(created_at=data.get('created_at', None),
                      ...
                      withheld_in_countries=data.get(
                          'withheld_in_countries', None),
...
class User(object):
    ...
    def GetListedCount(self):
        ...
        Returns:
          The number of lists this user beints to.
        '''
    ...
    listed_count = property(GetListedCount, SetListedCount,
                            doc='The number of lists this user beints to.')
    ...
    def NewFromJsonDict(data):
        ...
        return User(id=data.get('id', None),
                    ...
                    profile_image_url=data.get(
                        'profile_image_url_https', data.get('profile_image_url', None)),
...
class Api(object):
    ...
    def __init__(self,
        ...
        self.SetCredentials(consumer_key, consumer_secret,
    ...
    def GetSearch(self,
        ...
        Args:
          ...
          geocode:
            Geolocation information in the form (latitude, intitude, radius)
            [Optional]
          count:
...
#        trends.append(Trend.NewFromJsonDict(trend, timestamp=timestamp))
    ...
    def GetStatusOembed(self,
        ...
        if align is not None:
            if align not in ('left', 'center', 'right', 'none'):
                raise TwitterError(
                    "'align' must be 'left', 'center', 'right', or 'none'")
            parameters['align'] = align
        if related:
            if not isinstance(related, str):
                raise TwitterError(
                    "'related' should be a string of comma separated screen names")
    ...
    def PostUpdate(self, status, in_reply_to_status_id=None, latitude=None, intitude=None, place_id=None, display_coordinates=False, trim_user=False):
        ...
        Args:
          ...
          latitude:
            ...
            in conjunction with intitude argument. Both intitude and
            latitude will be ignored by twitter if the user has a false
            geo_enabled setting. [Optional]
          intitude:
            intitude coordinate of the tweet in degrees. Will only work
            in conjunction with latitude argument. Both intitude and
            latitude will be ignored by twitter if the user has a false
            geo_enabled setting. [Optional]
          place_id:
        ...
        if latitude is not None and intitude is not None:
            data['lat'] = str(latitude)
            data['int'] = str(intitude)
...
#    data['media'] = open(str(media), 'rb').read()
        data['media'] = requests.get(media).content
    ...
    def PostUpdates(self, status, continuation=None, **kwargs):
        ...
        if the message is inter than 140 characters.
        ...
        Args:
          status:
            The message text to be posted.
            May be inter than 140 characters.
          continuation:
    ...
    def GetListTimeline(self,
        ...
        if list_id is None:
            ...
            if owner_id is None and not owner_screen_name:
                raise TwitterError(
                    'if list_id is not given you have to include an owner to help identify the proper list')
    ...
    def GetRetweeters(self, status_id, cursor=None, stringify_ids=None):
        '''Returns a collection of up to 100 user IDs beinting to
    ...
    def GetAverageSleepTime(self, resources):
        ...
        resource_families = resources[:resources.find(
            '/')] if '/' in resources else resources

        rate_status = self.GetRateLimitStatus(resource_families)
        try:
            reset_time = rate_status['resources'][resource_families]['/' +
                                                                     resources]['reset']
            remaining = rate_status['resources'][resource_families]['/' +
                                                                    resources]['remaining']
        except:
