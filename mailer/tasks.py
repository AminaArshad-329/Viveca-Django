...
def prioritize():
    ...
    while True:
        while (
            Message.objects.high_priority().using("default").count()
            or Message.objects.medium_priority().using("default").count()
        ):
            while Message.objects.high_priority().using("default").count():
                for message in (
                    Message.objects.high_priority()
                    .using("default")
                    .order_by("when_added")
                ):
                    yield message
            while (
                Message.objects.high_priority().using("default").count() == 0
                and Message.objects.medium_priority().using("default").count()
            ):
                yield Message.objects.medium_priority().using("default").order_by(
                    "when_added"
                )[0]
        while (
            Message.objects.high_priority().using("default").count() == 0
            and Message.objects.medium_priority().using("default").count() == 0
            and Message.objects.low_priority().using("default").count()
        ):
            yield Message.objects.low_priority().using("default").order_by("when_added")[
                0
            ]
            ...
            try:
                ...
                logging.info(
                    ...
                    % (
                        ...
                        ", ".join(message.to_addresses).encode("utf-8"),
                    )
                )
                email = message.email
...
def send_loop():
    ...
    while True:
        while not Message.objects.all():
            logging.debug(
                "sleeping for %s seconds before checking queue again" % EMPTY_QUEUE_SLEEP
            )
